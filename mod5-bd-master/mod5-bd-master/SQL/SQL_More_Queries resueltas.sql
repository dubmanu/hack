...
!! 9. Display the Empno, Ename, job, Hiredate, Exp of all Mgrs 
	select empno,ename ,job,hiredate, round(datediff(current_date(),hiredate)/365,1) years_exp
...
!! 12. List the emps in the asc order of Designations of those joined after the second half of 1981. 
	select * from emp where hiredate > ('1981-06-30') order by job asc; 
...
!! 18. List the emps who are joined in the month of May 1981. 
	select * from emp where hiredate between '1981-05-01' and '1981-05-31'; 
...
YORK’,’DALLAS’) and e.deptno = d.deptno and e.empno in (select e.empno 
...
and empno in (select empno from emp where job like '%A%'and sal not like '%00' 
...
The subquery does the same as (The subquery does the same as (O
...
1500 
 union 

 select empno,ename, sal|| ‘BELOW 1500’ "SAL" from emp where sal < 
1500; (OR) 
...
average salary.List out the Name, Job, Salary of the emps in the department with the highest average salary.O
...
sal<2500 but not belongs to the month sep or nov workingList the empno,ename,sal,job,deptno&exp of all the emps belongs to dept 10 or 20 with an exp 6 to 10 y working under the same mgr with out comm. With a job not ending irrespective of the position with comm.>200 with exp>=7y and sal<2500 but not belongs to the month sep or nov workingO
under the mgr whose 
...
219) List the emps who are senior to mi List the emps who are senior to miO
...
between s.losal and s.hisal and 
 (s.grade in (select s.grade from emp e,salgrade s where e.sal 
between s.losal and s.hisal and e.ename = 'ADAMS') 
